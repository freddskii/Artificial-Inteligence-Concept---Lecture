...
class BackwardAnt:
    ...
    def calculate_path_metrics(self) -> Dict[str, float]:
        path_links = self.forward_ant.path_taken[1:]
        
        if not path_links:
            return {}
            
        total_quality = sum(hop['link_quality'] for hop in path_links)
        avg_quality = total_quality / len(path_links)
...
class UnifiedDrone:
    def __init__(self, drone_id: str, position: Position, drone_type: DroneType, 
        ...
        self.last_pheromone_update = time.time()
    ...
    def cleanup_old_data(self):
        ...
        with self.lock:
            if current_time - self.last_pheromone_update > 300:
                destinations_to_remove = []
                for dest_id, neighbors in self.pheromone_table_aco.items():
                    neighbors_to_remove = []
                    for neighbor_id, pheromone in neighbors.items():
                        if pheromone < 0.01:
                            neighbors_to_remove.append(neighbor_id)
                    for neighbor_id in neighbors_to_remove:
                        del neighbors[neighbor_id]
                    if not neighbors:
                        destinations_to_remove.append(dest_id)
                
                for dest_id in destinations_to_remove:
                    del self.pheromone_table_aco[dest_id]
                
                self.last_pheromone_update = current_time
            
            if len(self.route_cache_ga) > 100:
                oldest_keys = sorted(self.route_cache_ga.keys(), 
                                   key=lambda k: self.route_cache_ga[k]['timestamp'])[:50]
                for key in oldest_keys:
                    del self.route_cache_ga[key]
    ...
    def advanced_routing_decision_aco(self, available_neighbors: List[str], 
        ...
        for neighbor_id in available_neighbors:
            ...
            heuristic = 0.1
            if neighbor_id in drone_network:
    ...
    def find_route_ga(self, destination_id: str, drone_network: Dict[str, 'UnifiedDrone'],
        ...
        if population:
            ...
            result = best.route if best.fitness > 0.1 else None
            
            if result:
                self.route_cache_ga[cache_key] = {
                    'route': result,
                    'timestamp': time.time(),
    ...
    def crossover_ga(self, route1: List[str], route2: List[str], destination: str,
        ...
        if len(route1) < 2 or len(route2) < 2:
            return route1.copy() if route1 else None
        
        try:
            cut_point = min(len(route1) // 2, len(route2) // 2)
            child = route1[:cut_point]
            
            for node in route2[cut_point:]:
                if node not in child and self._is_valid_next_hop(child[-1], node, drone_network):
                    child.append(node)
                    if node == destination:
                        break
            
            if child[-1] != destination:
                if self._is_valid_next_hop(child[-1], destination, drone_network):
    ...
    def _is_valid_next_hop(self, current_id: str, next_id: str, drone_network: Dict[str, 'UnifiedDrone']) -> bool:
        if current_id not in drone_network:
            return False
        current_drone = drone_network[current_id]
        return next_id in current_drone.neighbor_drones
...
class ACOvsGAController:
    ...
    def visualize_test_round(self, test_pairs, aco_results, ga_results, test_number):
        ...
        plt.suptitle(f'Test Round at {self.simulation_time}s - ACO vs GA Comparison', 
    ...
    def visualize_route(self, result: RouteResult, algo_name: str, source: str, dest: str):

        fig, ax = plt.subplots(figsize=(10, 10))

        # Call your existing route drawer EXACTLY as-is
        self._draw_network_with_route(ax, result, algo_name, source, dest)
    ...
    def run_comparison_test(self, num_of_ga_repeat: int = 5):
        with self.network_lock:
            drone_ids = list(self.drones.keys())
        ...
        test_pairs = []
        for _ in range(min(num_of_ga_repeat, len(drone_ids) // 2)):
    ...
    def run_simulation(self, duration: int = 60, ga_repeats: int = 5):
        self.running = True
        start_time = time.time()
        self.last_test_time = 0
        ...
        try:
            while self.running and (time.time() - start_time) < duration:
                ...
                if self.simulation_time >= self.last_test_time + self.test_interval:
                    self.run_comparison_test(ga_repeats)
    ...
    def run_aco_only_test(self, source, dest, trials=10):
        results = []

        for _ in range(trials):
            res = self.test_aco_routing(source, dest)
            results.append(res)

        print("\n=== ACO ONLY TEST ===")
        success = sum(1 for r in results if r.success)
        print(f"Success rate: {success}/{trials} = {success/trials*100:.2f}%")
        print(f"Avg hops: {sum(r.hop_count for r in results if r.success)/success:.2f}")
        print(f"Avg quality: {sum(r.path_quality for r in results if r.success)/success:.2f}")
        print(f"Avg latency: {sum(r.latency for r in results if r.success)/success:.4f}s")

        return results

    def run_ga_only_test(self, source, dest, trials=10):
        results = []

        for _ in range(trials):
            res = self.test_ga_routing(source, dest)
            results.append(res)
...
class InteractivePlacementGUI:
    ...
    def start_placement_interface(self):
        ...
        self.fig.canvas.manager.set_window_title('ACO vs GA - Drone Placement')
...
def print_round_history(controller: ACOvsGAController):
